...
**IntentDetector** — детерминированный классификатор интентов (Intent) для think-пайплайна. Его задача — по входному `userText` определить, к какому типу относится сообщение пользователя (например: `GREETING`, `QUESTION`, `REQUEST`, …), и вернуть:
